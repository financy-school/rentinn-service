...
export class AnalyticsService {
  ...
  async getDashboardOverview(landlordId: number, queryDto: AnalyticsQueryDto) {
    ...
    const incomeStats = await this.getRentalIncomeStatistics(
      landlordId,
      start,
      end,
      propertyId,
  ...
  private async getPropertyStatistics(landlordId: number, propertyId?: number) {
    ...
    const valueQuery = this.roomRepository
      .createQueryBuilder('room')
      .innerJoin('room.property', 'property')
      .where('property.ownerId = :landlordId', { landlordId });
    ...
    return {
      ...
      averageRoomValue:
        totalRooms > 0 ? Number(totalValue?.totalValue || 0) / totalRooms : 0,
    };
  }
  ...
  private async getOccupancyStatistics(
    ...
    const propertyOccupancyQuery = this.propertyRepository
      .createQueryBuilder('property')
    ...
    if (propertyId) {
      propertyOccupancyQuery.andWhere('property.id = :propertyId', {
        propertyId,
      });
    }
    ...
    const propertyOccupancy = propertiesWithRooms.map((property) => ({
      ...
      occupancyRate:
        Number(property.totalRooms) > 0
          ? (Number(property.occupiedRooms) / Number(property.totalRooms)) * 100
          : 0,
    }));
  ...
  private async getPaymentStatistics(
    ...
    const paymentMethodsQuery = this.paymentRepository
      .createQueryBuilder('payment')
      .innerJoin('payment.rental', 'rental')
      .innerJoin('rental.room', 'room')
      .innerJoin('room.property', 'property')
    ...
    const paymentMethods = paymentMethodsResult.map((method) => ({
      ...
      percentage:
        Number(totalPaymentsResult.count) > 0
          ? (Number(method.count) / Number(totalPaymentsResult.count)) * 100
          : 0,
    }));
    ...
    return {
      ...
      latePayments: {
        ...
        percentage:
          Number(totalPaymentsResult?.count) > 0
            ? (Number(latePaymentsResult?.count || 0) /
                Number(totalPaymentsResult.count)) *
              100
            : 0,
      },
      paymentMethods,
  ...
  private async getTenantStatistics(landlordId: number) {
    ...
    const longTermTenantsQuery = this.userRepository
      .createQueryBuilder('user')
      .innerJoin('user.rentalsAsTenant', 'rental')
      .innerJoin('rental.room', 'room')
      .innerJoin('room.property', 'property')
    ...
    return {
      ...
      longTermTenants,
      tenantRetentionRate:
        totalTenants > 0 ? (longTermTenants / totalTenants) * 100 : 0,
      overdueTenantRate:
  ...
  async getRentalPerformanceMetrics(
    ...
    const propertyMetrics = await Promise.all(
      properties.map(async (property) => {
        ...
        const totalExpectedMonthly = rooms.reduce(
        ...
        // Calculate occupancy rate
        const occupiedRoomsQuery = this.roomRepository
          .createQueryBuilder('room')
          .innerJoin('room.rentals', 'rental')
          .where('room.propertyId = :propertyId', { propertyId: property.id })
          .andWhere('rental.isActive = :isActive', { isActive: true });
  ...
  async getTenantPaymentAnalytics(
    ...
    const tenants = await tenantsQuery.distinctOn(['user.id']).getMany();
    ...
    const tenantAnalytics = await Promise.all(
      tenants.map(async (tenant) => {
            ...
            if (payment.rentalId === rental.id) {
              ...
              if (payment.paymentDate <= dueDate) {
                ...
                const daysDiff = Math.floor(
                  (payment.paymentDate.getTime() - dueDate.getTime()) /
                    (1000 * 60 * 60 * 24),
                );
        ...
        const averageDaysToPay =
          paymentsWithDates > 0 ? totalDaysToPay / paymentsWithDates : 0;

        // Calculate current outstanding amount
        const outstandingAmount = rentals.reduce(
          (sum, rental) => sum + rental.outstandingAmount,
          0,
        );
  ...
  async getFinancialProjections(
    ...
    const propertyProjections = await Promise.all(
      properties.map(async (property) => {
        ...
        const currentOccupancyRate =
          rooms.length > 0 ? (roomsWithRentals / rooms.length) * 100 : 0;
        ...
        for (let i = 0; i < 12; i++) {
          ...
          if (projectedOccupancyRate < 95) {
            ...
            projectedOccupancyRate = Math.min(
              95,
              projectedOccupancyRate + increase,
            );
          }

          // Calculate projected income based on occupancy
          const projectedIncome =
            (totalPotentialMonthly * projectedOccupancyRate) / 100;
        ...
        const annualProjectedIncome = monthlyProjections.reduce(
          (sum, month) => sum + month.projectedIncome,
          0,
        );
          ...
          annualProjections: {
            ...
            capRate:
              property.totalArea > 0
                ? (netOperatingIncome / (totalPotentialMonthly * 12)) * 100
                : 0,
          },
        };
      }),
