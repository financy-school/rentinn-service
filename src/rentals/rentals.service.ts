...
export class RentalsService {
  ...
  async create(createRentalDto: CreateRentalDto): Promise<Rental> {
    ...
    const room = await this.propertiesService.findRoomById(createRentalDto.roomId);
    ...
    const hasVerifiedKyc = await this.kycService.hasVerifiedKyc(createRentalDto.tenantId);
    if (!hasVerifiedKyc) {
      throw new BadRequestException('Tenant must have at least one verified KYC document');
    }
   ...
   */
  async findAll(paginationDto: PaginationDto): Promise<PaginationResponse<Rental>> {
  ...
  async update(id: number, updateRentalDto: UpdateRentalDto): Promise<Rental> {
    ...
    if (updateRentalDto.startDate) {
      updateRentalDto.startDate = new Date(updateRentalDto.startDate).toISOString();
    }
   ...
   */
  async recordPayment(id: number, recordPaymentDto: RecordPaymentDto, recordedById: number): Promise<Payment> {
    const rental = await this.findOne(id);
    ...
    try {
      ...
      const newOutstanding = Math.max(0, currentOutstanding - recordPaymentDto.amount);
   ...
   * Get overdue rentals
   */
  async findOverdueRentals(paginationDto: PaginationDto): Promise<PaginationResponse<Rental>> {
    const { page, limit } = paginationDto;
    const skip = (page - 1) * limit;
